...
This is a small micro front end demo using [react](https://reactjs.org/) and [razzle](https://razzlejs.org/).

Starting point: [`src/server.js`](src/server.js).

- Header component is exposed on a dedicated route: /header
- Other components (micro-frontends) may be included in this app in the same way
- Right now all the css and js are compiled into one css and js 
file respectively. Hence there are dedicated routes for css and js.
- Any consumer intending to consume /header or more routes from this
application needs to consume the css and js endpoints exactly ONCE.
- Code splitting may help in isolating css and js files to related component routes.
E.g.: /header may come with header main content along with its required css and js.
The same hasn't been implemented yet.
- /home is the assembled page which includes header as a micro frontend.
For simplicity /home is served from this app. It may very well be served from a different
app.
- Micro-frontends are included via [Server Side Includes](https://kb.iu.edu/d/bfsa) which are
resolved by [nginx](nginx).
- The SSIs are resolved by nginx which sits in front of this app, and it expects full relative
urls. E.g.: This app is served by nginx under `/hello/`. However this app is unaware (and should
remain so) that its served under `/hello/`. This app only knows how to resolve urls that come
after `/hello/`. However if this app needs to perform any SSI (say /header) from a url served 
by this same app, then it needs to provide full url, i.e.: `/hello/header`, but again it shouldn't
know about `/hello`. This base path (`/hello`) is passed as a [header by nginx](nginx/nginx.conf),
and all relative urls are prefixed by the base path received from nginx. Hence, any external url 
changes shouldn't impact the code of this app.

---

## Trying out this demo

#### One time setup: `sh ./setup.sh`

#### Start up:
`sh ./run.sh`

Navigate to [http://localhost:8000/hello/](http://localhost:8000/hello/) to see the home page
which is being assembled from micro-frontend(s).
